import React, { useState } from 'react';
import { useHistory } from 'react-router-dom';
import { useFormik } from 'formik';
import {
  Container,
  Row,
  Col,
  Card,
  Form,
  Button,
} from 'react-bootstrap';
import * as yup from 'yup';
import axios from 'axios';
import { useAuth } from '../hooks/index.jsx';
import routes from '../routes.js';

const SignUpSchema = yup.object().shape({
  username: yup.string().min(3, 'От 3 до 20 символов').max(20, 'От 3 до 20 символов').required(),
  password: yup.string().min(6, 'Не менее 6 символов'),
  confirmPassword: yup.string().oneOf([yup.ref('password')], 'Пароли должны совпадать'),
});

const Signup = () => {
  const auth = useAuth();
  const history = useHistory();
  const [authFailed, setAuthFailed] = useState(false);

  const handlerSubmit = ({ username, password }, { setSubmitting }) => {
    setAuthFailed(false);
    const fetchContent = async () => {
      try {
        const user = { username, password };
        const res = await axios.post(routes.signupPath(), user);
        localStorage.setItem('userId', JSON.stringify(res.data));
        auth.logIn();
        history.replace('/');
      } catch (e) {
        if (e.isAxiosError && e.response.status === 409) {
          setAuthFailed(true);
          return;
        }
        throw e;
      }
    };
    fetchContent();
    setSubmitting(false);
  };
  const f = useFormik({
    initialValues: {
      username: '',
      password: '',
      confirmPassword: '',
    },
    validationSchema: SignUpSchema,
    onSubmit: handlerSubmit,
  });

  return (
    <Container fluid className="h-100">
      <Row className="justify-content-center align-content-center h-100">
        <Col xs={12} md={8} xxl={6}>
          <Card className="shadow-sm">
            <Card.Body className="p-5 flex-column flex-md-row d-flex align-items-center justify-content-center">
              <Col>
                <img src="data:image/jpeg;base64," className="rounded-circle" alt="Регистрация" />
              </Col>
              <Form className="w-50" onSubmit={f.handleSubmit}>
                <h1 className="text-center mb-4">Регистрация</h1>
                <Form.Group className="form-floating mb-3">
                  <Form.Label htmlFor="username">Имя пользователя</Form.Label>
                  <Form.Control
                    id="username"
                    name="username"
                    placeholder="Имя пользователя"
                    type="text"
                    required
                    onChange={f.handleChange}
                    readOnly={f.isSubmitting}
                    isInvalid={f.errors.username || authFailed}
                    value={f.values.username}
                  />
                  <Form.Control.Feedback type="invalid">{f.errors.username}</Form.Control.Feedback>
                </Form.Group>
                <Form.Group className="form-floating mb-3">
                  <Form.Label htmlFor="password">Пароль</Form.Label>
                  <Form.Control
                    id="password"
                    name="password"
                    type="password"
                    placeholder="Пароль"
                    required
                    onChange={f.handleChange}
                    readOnly={f.isSubmitting}
                    isInvalid={f.errors.password || authFailed}
                    value={f.values.password}
                  />
                  <Form.Control.Feedback type="invalid">{f.errors.password}</Form.Control.Feedback>
                </Form.Group>
                <Form.Group className="form-floating mb-4">
                  <Form.Label htmlFor="password">Подтвердите пароль</Form.Label>
                  <Form.Control
                    id="confirmPassword"
                    name="confirmPassword"
                    type="password"
                    placeholder="Подтвердите пароль"
                    required
                    onChange={f.handleChange}
                    readOnly={f.isSubmitting}
                    isInvalid={f.errors.confirmPassword || authFailed}
                    value={f.values.confirmPassword}
                  />
                  <Form.Control.Feedback type="invalid">{authFailed ? 'Такой пользователь уже существует' : f.errors.confirmPassword}</Form.Control.Feedback>
                </Form.Group>
                <Button
                  type="submit"
                  variant="outline-primary"
                  className="w-100"
                  disabled={f.isSubmitting}
                >
                  Зарегистрироваться
                </Button>
              </Form>
            </Card.Body>
          </Card>
        </Col>
      </Row>
    </Container>
  );
};

export default Signup;
